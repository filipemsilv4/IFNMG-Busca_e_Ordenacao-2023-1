...
int main(int argc, char **argv)
{
    ...
    else if (options[2].second)
    {
        ...
        if (options[1].second)
        {
            ...
            quicksort(*list, 0, list->size() - 1, options[0].second);
...
void display_options(vector<pair<string, bool>> &options, int max_option_length)
{
    ...
    while (!done)
    {
        ...
        case '\n': // User pressed enter
            ...
            {
                // Show the state of the list at each iteration
                options[selected_option].second = !options[selected_option].second;
            }
            else if (selected_option == 1)
            {
                // Show the execution time of the algorithm
                options[selected_option].second = !options[selected_option].second;
            }
